...
Deriva también de los problemas encontrados con el tipo \code{Value}. La
...
\lifetimes inexplicables (ver Figura~\ref{fig:errors}). Estuve bloqueado con
dicho problema durante mucho tiempo, así que tras comentárselo a mis mentores,
Heinz me ayudó a reproducir el problema de forma mínima. Por alguna razón que
todavía desconocíamos, dos tipos supuestamente equivalentes diferían a la hora
de compilar:
...
siguiente. Se tienen dos \lifetimes \emph{sin relación entre sí}, \code{'short}
y \code{'long}. La estructura \code{Foo} que se pasa como parámetro tiene la
\lifetime \code{'short}, pero dentro de la función se le intenta asignar una
\lifetime \code{'long}, que es imposible porque el compilador no sabe cuál tiene
un tiempo de vida mayor. Asignarle una \lifetime que viva más de lo que debe
significiaría que se podría seguir usando después de que \code{'short} acabe, es
decir, después de que \code{Foo} haya sido destruido. Finalmente, esto causaría
inconsistencias en memoria porque nuestra variable de tipo \code{Foo} ya no
existe, pero se está intentando acceder a ella.
...
Solucionarlo es tan simple como indicar que \code{'short} tiene al menos el
mismo tiempo de vida que \code{'long}. Es decir, que no se podría dar el caso de
que \code{Foo} es usado después de destruirse:
...
Por tanto, uno pensaría que tiene que ver con la implementación del operador
\code{==}, que se delega al \trait \code{PartialEq}, así que dediqué tiempo
intentando encontrar la diferencia. Y en efecto, al usar \emph{exactamente la
misma implementación} para \code{RCow} que para \code{Cow}, compilaba
correctamente.

Sin embargo, tras cambiar \code{left == right} por \code{left.cmp(right)}, se
repetía el problema, incluso con la misma implementación de \code{PartialOrd},
el \trait con el método \code{cmp}. No fue hasta que pedí ayuda en el servidor
de Discord de Rust hasta que descubrí que el verdadero problema era un término
llamado la \emph{varianza} de \code{RCow}.
