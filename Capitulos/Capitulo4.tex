...
eficientes porque no impone casi ningún coste adicinal tras el cargado de la
librería dinámica.

La \crate principal para este método es \cratelink{libloading}, aunque también
existen las menos conocidas \cratelink{dlopen} y \cratelink{sharedlib}, con
...
generalmente precedidos por la letra \code{R}. Por tanto, en vez de
\rust{Vec<T>}, podremos usar \rust{RVec<T>}; en caso contrario habría que
recurrir a punteros \rust{*const T} o tendríamos que reescribir los tipos desde
cero nosotros. También da soporte para librerías externas muy conocidas en la
comunidad, como \cratelink{crossbeam} o \cratelink{serde_json}. La
Figura~\ref{fig:rustabi_stable} demuestra parte de la simplificación respecto al
código en la Figura~\ref{fig:rustffi}.

Existen algunas alternativas o extensiones para el cargado dinámico como
...
el aumento de complejidad deberá tenerse en cuenta en la decisión final.
